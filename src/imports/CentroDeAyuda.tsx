import svgPaths from "./svg-ouudiuwxq0";
import imgBigdamAzul1 from "@/assets/618631a906a3f14879ebf268c012439e9a59550d.png";
import { imgVerified, imgLocalShipping } from "./svg-sqtzw";

function Frame4() {
  return (
    <div className="content-stretch flex gap-[4px] items-center relative shrink-0">
      <div className="relative shrink-0 size-[24px]" data-name="verified">
        <div className="absolute inset-[8.89%_6.97%] mask-alpha mask-intersect mask-no-clip mask-no-repeat mask-position-[-1.673px_-2.135px] mask-size-[24px_24px]" data-name="verified" style={{ maskImage: `url('${imgVerified}')` }}>
          <svg className="absolute block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 20.6535 19.7305">
            <path d={svgPaths.p36ce9c00} fill="var(--fill-0, white)" id="verified" />
          </svg>
        </div>
      </div>
      <div className="flex flex-col font-['Lato:Regular',sans-serif] justify-center leading-[0] not-italic relative shrink-0 text-[16px] text-white whitespace-nowrap">
        <p className="leading-[1.5]">Más de 30 años de experiencia</p>
      </div>
    </div>
  );
}

function Frame3() {
  return (
    <div className="content-stretch flex gap-[4px] items-center relative shrink-0">
      <div className="relative shrink-0 size-[24px]" data-name="local_shipping">
        <div className="absolute inset-[18.75%_5.61%_18.51%_5.61%] mask-alpha mask-intersect mask-no-clip mask-no-repeat mask-position-[-1.347px_-4.5px] mask-size-[24px_24px]" data-name="local_shipping" style={{ maskImage: `url('${imgVerified}')` }}>
          <svg className="absolute block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 21.3075 15.0577">
            <path d={svgPaths.p21ffd200} fill="var(--fill-0, white)" id="local_shipping" />
          </svg>
        </div>
      </div>
      <div className="flex flex-col font-['Lato:Regular',sans-serif] h-[36px] justify-center leading-[0] not-italic relative shrink-0 text-[16px] text-white w-[157px]">
        <p className="leading-[1.5] whitespace-pre-wrap">{`Envió  a nivel nacional`}</p>
      </div>
    </div>
  );
}

function Frame2() {
  return (
    <div className="absolute bg-[#e30613] content-stretch flex h-[48px] items-center justify-between left-0 px-[64px] py-[6px] top-0 w-[1440px]">
      <Frame4 />
      <Frame3 />
    </div>
  );
}

function Group7() {
  return (
    <div className="absolute contents left-0 top-0">
      <Frame2 />
    </div>
  );
}

function Frame() {
  return (
    <div className="bg-[#fafafa] content-stretch flex gap-[15px] items-center px-[16px] py-[4px] relative rounded-[12px] shrink-0">
      <div aria-hidden="true" className="absolute border border-[#302c67] border-solid inset-0 pointer-events-none rounded-[12px]" />
      <div className="flex flex-col font-['Lato:Regular',sans-serif] h-[36px] justify-center leading-[0] not-italic relative shrink-0 text-[#6b6b7b] text-[14px] tracking-[0.0014px] w-[555px]">
        <p className="leading-[1.5]">Buscar por marca, modelo, año o motor...</p>
      </div>
      <div className="relative shrink-0 size-[24px]" data-name="search">
        <div className="absolute inset-[14.1%_14.18%_14.26%_14.18%] mask-alpha mask-intersect mask-no-clip mask-no-repeat mask-position-[-3.404px_-3.385px] mask-size-[24px_24px]" data-name="search" style={{ maskImage: `url('${imgVerified}')` }}>
          <svg className="absolute block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 17.192 17.1923">
            <path d={svgPaths.p34e8e200} fill="var(--fill-0, #302C67)" id="search" />
          </svg>
        </div>
      </div>
    </div>
  );
}

function Frame5() {
  return (
    <div className="col-1 content-stretch flex items-center justify-between ml-0 mt-0 relative row-1 w-[303px]">
      <div className="bg-white content-stretch flex gap-[4px] h-[52px] items-center justify-center p-[16px] relative rounded-[12px] shrink-0" data-name="Botones">
        <p className="font-['Lato:Regular',sans-serif] leading-[1.5] not-italic relative shrink-0 text-[#302c67] text-[16px] whitespace-nowrap">Mi carrito</p>
        <div className="relative shrink-0 size-[24px]" data-name="shopping_cart">
          <div className="absolute inset-[9.37%_12.47%_10.18%_6.25%] mask-alpha mask-intersect mask-no-clip mask-no-repeat mask-position-[-1.5px_-2.25px] mask-size-[24px_24px]" data-name="shopping_cart" style={{ maskImage: `url('${imgVerified}')` }}>
            <svg className="absolute block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 19.5077 19.3078">
              <path d={svgPaths.p3761b200} fill="var(--fill-0, #302C67)" id="shopping_cart" />
            </svg>
          </div>
        </div>
      </div>
      <div className="flex h-[24px] items-center justify-center relative shrink-0 w-0" style={{ "--transform-inner-width": "1200", "--transform-inner-height": "19" } as React.CSSProperties}>
        <div className="flex-none rotate-90">
          <div className="h-0 relative w-[24px]">
            <div className="absolute inset-[-1px_0_0_0]">
              <svg className="block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 24 1">
                <line id="Line 29" stroke="var(--stroke-0, #302C67)" x2="24" y1="0.5" y2="0.5" />
              </svg>
            </div>
          </div>
        </div>
      </div>
      <div className="bg-white content-stretch flex gap-[4px] h-[52px] items-center justify-center p-[16px] relative rounded-[12px] shrink-0 w-[124px]" data-name="Botones">
        <p className="font-['Lato:Regular',sans-serif] leading-[1.5] not-italic relative shrink-0 text-[#302c67] text-[16px] whitespace-nowrap">Mi perfil</p>
        <div className="relative shrink-0 size-[24px]" data-name="account_circle">
          <div className="absolute inset-[10.42%] mask-alpha mask-intersect mask-no-clip mask-no-repeat mask-position-[-2.5px_-2.5px] mask-size-[24px_24px]" data-name="account_circle" style={{ maskImage: `url('${imgVerified}')` }}>
            <svg className="absolute block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 19 19">
              <path d={svgPaths.p2155cd80} fill="var(--fill-0, #302C67)" id="account_circle" />
            </svg>
          </div>
        </div>
      </div>
    </div>
  );
}

function Group8() {
  return (
    <div className="grid-cols-[max-content] grid-rows-[max-content] inline-grid leading-[0] place-items-start relative shrink-0">
      <Frame5 />
    </div>
  );
}

function Frame1() {
  return (
    <div className="bg-[#fafafa] content-stretch flex items-center justify-between px-[16px] py-[4px] relative rounded-[12px] shrink-0 w-[1308px]">
      <div aria-hidden="true" className="absolute border border-[#302c67] border-solid inset-0 pointer-events-none rounded-[12px]" />
      <div className="flex flex-col font-['Lato:Regular',sans-serif] h-[36px] justify-center leading-[0] not-italic relative shrink-0 text-[#6b6b7b] text-[14px] tracking-[0.0014px] w-[555px]">
        <p className="leading-[1.5]">Buscar por marca, modelo, año o motor...</p>
      </div>
      <div className="relative shrink-0 size-[24px]" data-name="search">
        <div className="absolute inset-[14.1%_14.18%_14.26%_14.18%] mask-alpha mask-intersect mask-no-clip mask-no-repeat mask-position-[-3.404px_-3.385px] mask-size-[24px_24px]" data-name="search" style={{ maskImage: `url('${imgVerified}')` }}>
          <svg className="absolute block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 17.192 17.1923">
            <path d={svgPaths.p34e8e200} fill="var(--fill-0, #302C67)" id="search" />
          </svg>
        </div>
      </div>
    </div>
  );
}

function Frame44() {
  return (
    <div className="content-stretch flex gap-[4px] items-center relative shrink-0 w-full">
      <div className="relative shrink-0 size-[24px]" data-name="verified">
        <div className="absolute inset-[8.89%_6.97%] mask-alpha mask-intersect mask-no-clip mask-no-repeat mask-position-[-1.673px_-2.135px] mask-size-[24px_24px]" data-name="verified" style={{ maskImage: `url('${imgVerified}')` }}>
          <svg className="absolute block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 20.6535 19.7305">
            <path d={svgPaths.p36ce9c00} fill="var(--fill-0, #302C67)" id="verified" />
          </svg>
        </div>
      </div>
      <p className="font-['Lato:Bold',sans-serif] leading-[1.3] not-italic relative shrink-0 text-[#302c67] text-[24px] w-[722px]">Sobre los productos</p>
    </div>
  );
}

function Frame37() {
  return (
    <div className="content-stretch flex h-[48px] items-center justify-between relative shrink-0 w-full">
      <p className="font-['Lato:Regular',sans-serif] h-[24px] leading-[1.5] not-italic relative shrink-0 text-[#302c67] text-[16px] w-[515px]">¿Qué tipo de productos puedo encontrar en la tienda?</p>
      <div className="relative shrink-0 size-[24px]" data-name="horizontal_rule">
        <div className="absolute inset-[46.88%_18.75%] mask-alpha mask-intersect mask-no-clip mask-no-repeat mask-position-[-4.5px_-11.25px] mask-size-[24px_24px]" data-name="horizontal_rule" style={{ maskImage: `url('${imgVerified}')` }}>
          <svg className="absolute block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 15 1.5">
            <path d="M0 1.5V0H15V1.5H0Z" fill="var(--fill-0, #302C67)" id="horizontal_rule" />
          </svg>
        </div>
      </div>
    </div>
  );
}

function Frame38() {
  return (
    <div className="content-stretch flex flex-col gap-[16px] items-start py-[12px] relative shrink-0 w-full">
      <div aria-hidden="true" className="absolute border-[#bfbed0] border-b border-solid inset-0 pointer-events-none" />
      <Frame37 />
      <p className="font-['Lato:Regular',sans-serif] leading-[1.5] not-italic relative shrink-0 text-[#6b6b7b] text-[16px] w-full">Ofrecemos una amplia variedad de productos cuidadosamente seleccionados, con calidad garantizada y actualizaciones constantes en nuestro catálogo.</p>
    </div>
  );
}

function Frame40() {
  return (
    <div className="content-stretch flex h-[48px] items-center justify-between relative shrink-0 w-full">
      <p className="font-['Lato:Regular',sans-serif] h-[24px] leading-[1.5] not-italic relative shrink-0 text-[#302c67] text-[16px] w-[515px]">¿Los productos cuentan con garantía?</p>
      <div className="relative shrink-0 size-[24px]" data-name="add_2">
        <div className="absolute inset-[14.58%] mask-alpha mask-intersect mask-no-clip mask-no-repeat mask-position-[-3.5px_-3.5px] mask-size-[24px_24px]" data-name="add_2" style={{ maskImage: `url('${imgVerified}')` }}>
          <svg className="absolute block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 17 17">
            <path d={svgPaths.p1ad2a780} fill="var(--fill-0, #302C67)" id="add_2" />
          </svg>
        </div>
      </div>
    </div>
  );
}

function Frame39() {
  return (
    <div className="content-stretch flex flex-col items-start py-[12px] relative shrink-0 w-full">
      <div aria-hidden="true" className="absolute border-[#bfbed0] border-b border-solid inset-0 pointer-events-none" />
      <Frame40 />
    </div>
  );
}

function Frame42() {
  return (
    <div className="content-stretch flex h-[48px] items-center justify-between relative shrink-0 w-full">
      <p className="font-['Lato:Regular',sans-serif] leading-[1.5] not-italic relative shrink-0 text-[#302c67] text-[16px] w-[515px]">¿Los productos son originales?</p>
      <div className="relative shrink-0 size-[24px]" data-name="add_2">
        <div className="absolute inset-[14.58%] mask-alpha mask-intersect mask-no-clip mask-no-repeat mask-position-[-3.5px_-3.5px] mask-size-[24px_24px]" data-name="add_2" style={{ maskImage: `url('${imgVerified}')` }}>
          <svg className="absolute block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 17 17">
            <path d={svgPaths.p1ad2a780} fill="var(--fill-0, #302C67)" id="add_2" />
          </svg>
        </div>
      </div>
    </div>
  );
}

function Frame41() {
  return (
    <div className="content-stretch flex flex-col items-start py-[12px] relative shrink-0 w-full">
      <div aria-hidden="true" className="absolute border-[#bfbed0] border-b border-solid inset-0 pointer-events-none" />
      <Frame42 />
    </div>
  );
}

function Frame47() {
  return (
    <div className="bg-[#f0f0f2] relative rounded-[24px] shrink-0 w-full">
      <div className="content-stretch flex flex-col gap-[24px] items-start px-[24px] py-[48px] relative w-full">
        <Frame44 />
        <Frame38 />
        <Frame39 />
        <Frame41 />
      </div>
    </div>
  );
}

function Frame45() {
  return (
    <div className="content-stretch flex gap-[4px] items-center relative shrink-0 w-full">
      <div className="relative shrink-0 size-[24px]" data-name="credit_card">
        <div className="absolute inset-[18.75%_10.42%] mask-alpha mask-intersect mask-no-clip mask-no-repeat mask-position-[-2.5px_-4.5px] mask-size-[24px_24px]" data-name="credit_card" style={{ maskImage: `url('${imgVerified}')` }}>
          <svg className="absolute block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 19 15">
            <path d={svgPaths.p31b34200} fill="var(--fill-0, #302C67)" id="credit_card" />
          </svg>
        </div>
      </div>
      <p className="font-['Lato:Bold',sans-serif] leading-[1.3] not-italic relative shrink-0 text-[#302c67] text-[24px] w-[722px]">Pagos</p>
    </div>
  );
}

function Frame48() {
  return (
    <div className="content-stretch flex h-[48px] items-center justify-between relative shrink-0 w-full">
      <p className="font-['Lato:Regular',sans-serif] h-[24px] leading-[1.5] not-italic relative shrink-0 text-[#302c67] text-[16px] w-[515px]">¿Qué métodos de pago están disponibles?</p>
      <div className="relative shrink-0 size-[24px]" data-name="horizontal_rule">
        <div className="absolute inset-[46.88%_18.75%] mask-alpha mask-intersect mask-no-clip mask-no-repeat mask-position-[-4.5px_-11.25px] mask-size-[24px_24px]" data-name="horizontal_rule" style={{ maskImage: `url('${imgVerified}')` }}>
          <svg className="absolute block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 15 1.5">
            <path d="M0 1.5V0H15V1.5H0Z" fill="var(--fill-0, #302C67)" id="horizontal_rule" />
          </svg>
        </div>
      </div>
    </div>
  );
}

function Frame43() {
  return (
    <div className="content-stretch flex flex-col gap-[19px] items-start py-[12px] relative shrink-0 w-full">
      <div aria-hidden="true" className="absolute border-[#bfbed0] border-b border-solid inset-0 pointer-events-none" />
      <Frame48 />
      <p className="font-['Lato:Regular',sans-serif] leading-[1.5] not-italic relative shrink-0 text-[#6b6b7b] text-[16px] w-full">Aceptamos diferentes métodos de pago como transferencias bancarias, pagos con tarjeta y opciones en cuotas (según disponibilidad).</p>
    </div>
  );
}

function Frame50() {
  return (
    <div className="content-stretch flex h-[48px] items-center justify-between relative shrink-0 w-full">
      <p className="font-['Lato:Regular',sans-serif] h-[24px] leading-[1.5] not-italic relative shrink-0 text-[#302c67] text-[16px] w-[515px]">¿Es seguro pagar en la plataforma?</p>
      <div className="relative shrink-0 size-[24px]" data-name="add_2">
        <div className="absolute inset-[14.58%] mask-alpha mask-intersect mask-no-clip mask-no-repeat mask-position-[-3.5px_-3.5px] mask-size-[24px_24px]" data-name="add_2" style={{ maskImage: `url('${imgVerified}')` }}>
          <svg className="absolute block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 17 17">
            <path d={svgPaths.p1ad2a780} fill="var(--fill-0, #302C67)" id="add_2" />
          </svg>
        </div>
      </div>
    </div>
  );
}

function Frame49() {
  return (
    <div className="content-stretch flex flex-col items-start py-[12px] relative shrink-0 w-full">
      <div aria-hidden="true" className="absolute border-[#bfbed0] border-b border-solid inset-0 pointer-events-none" />
      <Frame50 />
    </div>
  );
}

function Frame52() {
  return (
    <div className="content-stretch flex h-[48px] items-center justify-between relative shrink-0 w-full">
      <p className="font-['Lato:Regular',sans-serif] leading-[1.5] not-italic relative shrink-0 text-[#302c67] text-[16px] w-[515px]">¿Puedo pagar en cuotas?</p>
      <div className="relative shrink-0 size-[24px]" data-name="add_2">
        <div className="absolute inset-[14.58%] mask-alpha mask-intersect mask-no-clip mask-no-repeat mask-position-[-3.5px_-3.5px] mask-size-[24px_24px]" data-name="add_2" style={{ maskImage: `url('${imgVerified}')` }}>
          <svg className="absolute block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 17 17">
            <path d={svgPaths.p1ad2a780} fill="var(--fill-0, #302C67)" id="add_2" />
          </svg>
        </div>
      </div>
    </div>
  );
}

function Frame51() {
  return (
    <div className="content-stretch flex flex-col items-start py-[12px] relative shrink-0 w-full">
      <div aria-hidden="true" className="absolute border-[#bfbed0] border-b border-solid inset-0 pointer-events-none" />
      <Frame52 />
    </div>
  );
}

function Frame46() {
  return (
    <div className="bg-[#f0f0f2] relative rounded-[24px] shrink-0 w-full">
      <div className="content-stretch flex flex-col gap-[24px] items-start px-[24px] py-[48px] relative w-full">
        <Frame45 />
        <Frame43 />
        <Frame49 />
        <Frame51 />
      </div>
    </div>
  );
}

function Frame55() {
  return (
    <div className="content-stretch flex gap-[4px] items-center relative shrink-0 w-full">
      <div className="relative shrink-0 size-[24px]" data-name="package_2">
        <div className="absolute inset-[9.49%_14.58%] mask-alpha mask-intersect mask-no-clip mask-no-repeat mask-position-[-3.5px_-2.277px] mask-size-[24px_24px]" data-name="package_2" style={{ maskImage: `url('${imgVerified}')` }}>
          <svg className="absolute block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 17 19.446">
            <path d={svgPaths.p3290d0f0} fill="var(--fill-0, #302C67)" id="package_2" />
          </svg>
        </div>
      </div>
      <p className="font-['Lato:Bold',sans-serif] leading-[1.3] not-italic relative shrink-0 text-[#302c67] text-[24px] w-[722px]">Envíos</p>
    </div>
  );
}

function Frame57() {
  return (
    <div className="content-stretch flex h-[48px] items-center justify-between relative shrink-0 w-full">
      <p className="font-['Lato:Regular',sans-serif] h-[24px] leading-[1.5] not-italic relative shrink-0 text-[#302c67] text-[16px] w-[515px]">¿Realizan envíos a todo el país?</p>
      <div className="relative shrink-0 size-[24px]" data-name="horizontal_rule">
        <div className="absolute inset-[46.88%_18.75%] mask-alpha mask-intersect mask-no-clip mask-no-repeat mask-position-[-4.5px_-11.25px] mask-size-[24px_24px]" data-name="horizontal_rule" style={{ maskImage: `url('${imgVerified}')` }}>
          <svg className="absolute block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 15 1.5">
            <path d="M0 1.5V0H15V1.5H0Z" fill="var(--fill-0, #302C67)" id="horizontal_rule" />
          </svg>
        </div>
      </div>
    </div>
  );
}

function Frame56() {
  return (
    <div className="content-stretch flex flex-col gap-[19px] items-start py-[12px] relative shrink-0 w-full">
      <div aria-hidden="true" className="absolute border-[#bfbed0] border-b border-solid inset-0 pointer-events-none" />
      <Frame57 />
      <p className="font-['Lato:Regular',sans-serif] leading-[1.5] not-italic relative shrink-0 text-[#6b6b7b] text-[16px] w-full">Sí, realizamos envíos a nivel nacional. Las zonas disponibles y costos se detallan durante el proceso de compra.</p>
    </div>
  );
}

function Frame59() {
  return (
    <div className="content-stretch flex h-[48px] items-center justify-between relative shrink-0 w-full">
      <p className="font-['Lato:Regular',sans-serif] h-[24px] leading-[1.5] not-italic relative shrink-0 text-[#302c67] text-[16px] w-[515px]">¿Cuánto tarda en llegar mi pedido?</p>
      <div className="relative shrink-0 size-[24px]" data-name="add_2">
        <div className="absolute inset-[14.58%] mask-alpha mask-intersect mask-no-clip mask-no-repeat mask-position-[-3.5px_-3.5px] mask-size-[24px_24px]" data-name="add_2" style={{ maskImage: `url('${imgVerified}')` }}>
          <svg className="absolute block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 17 17">
            <path d={svgPaths.p1ad2a780} fill="var(--fill-0, #302C67)" id="add_2" />
          </svg>
        </div>
      </div>
    </div>
  );
}

function Frame58() {
  return (
    <div className="content-stretch flex flex-col items-start py-[12px] relative shrink-0 w-full">
      <div aria-hidden="true" className="absolute border-[#bfbed0] border-b border-solid inset-0 pointer-events-none" />
      <Frame59 />
    </div>
  );
}

function Frame61() {
  return (
    <div className="content-stretch flex h-[48px] items-center justify-between relative shrink-0 w-full">
      <p className="font-['Lato:Regular',sans-serif] leading-[1.5] not-italic relative shrink-0 text-[#302c67] text-[16px] w-[515px]">¿Puedo hacer seguimiento a mi pedido?</p>
      <div className="relative shrink-0 size-[24px]" data-name="add_2">
        <div className="absolute inset-[14.58%] mask-alpha mask-intersect mask-no-clip mask-no-repeat mask-position-[-3.5px_-3.5px] mask-size-[24px_24px]" data-name="add_2" style={{ maskImage: `url('${imgVerified}')` }}>
          <svg className="absolute block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 17 17">
            <path d={svgPaths.p1ad2a780} fill="var(--fill-0, #302C67)" id="add_2" />
          </svg>
        </div>
      </div>
    </div>
  );
}

function Frame60() {
  return (
    <div className="content-stretch flex flex-col items-start py-[12px] relative shrink-0 w-full">
      <div aria-hidden="true" className="absolute border-[#bfbed0] border-b border-solid inset-0 pointer-events-none" />
      <Frame61 />
    </div>
  );
}

function Frame53() {
  return (
    <div className="bg-[#f0f0f2] relative rounded-[24px] shrink-0 w-full">
      <div className="content-stretch flex flex-col gap-[24px] items-start px-[24px] py-[48px] relative w-full">
        <Frame55 />
        <Frame56 />
        <Frame58 />
        <Frame60 />
      </div>
    </div>
  );
}

function Frame63() {
  return (
    <div className="content-stretch flex gap-[4px] items-center relative shrink-0 w-full">
      <div className="relative shrink-0 size-[24px]" data-name="restart_alt">
        <div className="absolute inset-[12.9%_18.75%_14.79%_18.75%] mask-alpha mask-intersect mask-no-clip mask-no-repeat mask-position-[-4.5px_-3.096px] mask-size-[24px_24px]" data-name="restart_alt" style={{ maskImage: `url('${imgVerified}')` }}>
          <svg className="absolute block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 15 17.3538">
            <path d={svgPaths.p1f772b00} fill="var(--fill-0, #302C67)" id="restart_alt" />
          </svg>
        </div>
      </div>
      <p className="font-['Lato:Bold',sans-serif] leading-[1.3] not-italic relative shrink-0 text-[#302c67] text-[24px] w-[722px]">Cambios y devoluciones</p>
    </div>
  );
}

function Frame65() {
  return (
    <div className="content-stretch flex h-[48px] items-center justify-between relative shrink-0 w-full">
      <p className="font-['Lato:Regular',sans-serif] h-[24px] leading-[1.5] not-italic relative shrink-0 text-[#302c67] text-[16px] w-[515px]">¿Puedo cambiar o devolver un producto?</p>
      <div className="relative shrink-0 size-[24px]" data-name="horizontal_rule">
        <div className="absolute inset-[46.88%_18.75%] mask-alpha mask-intersect mask-no-clip mask-no-repeat mask-position-[-4.5px_-11.25px] mask-size-[24px_24px]" data-name="horizontal_rule" style={{ maskImage: `url('${imgVerified}')` }}>
          <svg className="absolute block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 15 1.5">
            <path d="M0 1.5V0H15V1.5H0Z" fill="var(--fill-0, #302C67)" id="horizontal_rule" />
          </svg>
        </div>
      </div>
    </div>
  );
}

function Frame64() {
  return (
    <div className="content-stretch flex flex-col gap-[19px] items-start py-[12px] relative shrink-0 w-full">
      <div aria-hidden="true" className="absolute border-[#bfbed0] border-b border-solid inset-0 pointer-events-none" />
      <Frame65 />
      <p className="font-['Lato:Regular',sans-serif] leading-[1.5] not-italic relative shrink-0 text-[#6b6b7b] text-[16px] w-full">Sí, aceptamos cambios o devoluciones dentro de un plazo determinado, siempre que el producto esté en perfectas condiciones.</p>
    </div>
  );
}

function Frame67() {
  return (
    <div className="content-stretch flex h-[48px] items-center justify-between relative shrink-0 w-full">
      <p className="font-['Lato:Regular',sans-serif] h-[24px] leading-[1.5] not-italic relative shrink-0 text-[#302c67] text-[16px] w-[515px]">¿Qué hago si mi producto llega dañado o incorrecto?</p>
      <div className="relative shrink-0 size-[24px]" data-name="add_2">
        <div className="absolute inset-[14.58%] mask-alpha mask-intersect mask-no-clip mask-no-repeat mask-position-[-3.5px_-3.5px] mask-size-[24px_24px]" data-name="add_2" style={{ maskImage: `url('${imgVerified}')` }}>
          <svg className="absolute block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 17 17">
            <path d={svgPaths.p1ad2a780} fill="var(--fill-0, #302C67)" id="add_2" />
          </svg>
        </div>
      </div>
    </div>
  );
}

function Frame66() {
  return (
    <div className="content-stretch flex flex-col items-start py-[12px] relative shrink-0 w-full">
      <div aria-hidden="true" className="absolute border-[#bfbed0] border-b border-solid inset-0 pointer-events-none" />
      <Frame67 />
    </div>
  );
}

function Frame69() {
  return (
    <div className="content-stretch flex h-[48px] items-center justify-between relative shrink-0 w-full">
      <p className="font-['Lato:Regular',sans-serif] leading-[1.5] not-italic relative shrink-0 text-[#302c67] text-[16px] w-[515px]">¿Quién cubre los costos de envío en una devolución?</p>
      <div className="relative shrink-0 size-[24px]" data-name="add_2">
        <div className="absolute inset-[14.58%] mask-alpha mask-intersect mask-no-clip mask-no-repeat mask-position-[-3.5px_-3.5px] mask-size-[24px_24px]" data-name="add_2" style={{ maskImage: `url('${imgVerified}')` }}>
          <svg className="absolute block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 17 17">
            <path d={svgPaths.p1ad2a780} fill="var(--fill-0, #302C67)" id="add_2" />
          </svg>
        </div>
      </div>
    </div>
  );
}

function Frame68() {
  return (
    <div className="content-stretch flex flex-col items-start py-[12px] relative shrink-0 w-full">
      <div aria-hidden="true" className="absolute border-[#bfbed0] border-b border-solid inset-0 pointer-events-none" />
      <Frame69 />
    </div>
  );
}

function Frame62() {
  return (
    <div className="bg-[#f0f0f2] relative rounded-[24px] shrink-0 w-full">
      <div className="content-stretch flex flex-col gap-[24px] items-start px-[24px] py-[48px] relative w-full">
        <Frame63 />
        <Frame64 />
        <Frame66 />
        <Frame68 />
      </div>
    </div>
  );
}

function Frame34() {
  return (
    <div className="content-stretch flex flex-col gap-[4px] items-center relative shrink-0 w-full">
      <div className="relative rounded-[4px] shrink-0 size-[64px]" data-name="local_shipping">
        <div className="absolute inset-[18.75%_5.61%_18.51%_5.61%] mask-alpha mask-intersect mask-no-clip mask-no-repeat mask-position-[-1.347px_-4.5px] mask-size-[24px_24px]" data-name="local_shipping" style={{ maskImage: `url('${imgLocalShipping}')` }}>
          <svg className="absolute block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 56.82 40.154">
            <path d={svgPaths.p2bc04d00} fill="var(--fill-0, #302C67)" id="local_shipping" />
          </svg>
        </div>
      </div>
      <p className="font-['Lato:Bold',sans-serif] leading-[1.3] min-w-full not-italic relative shrink-0 text-[#262352] text-[24px] text-center w-[min-content]">Envió rápido</p>
    </div>
  );
}

function Frame7() {
  return (
    <div className="bg-white content-stretch flex flex-col gap-[12px] items-center justify-center p-[24px] relative rounded-[12px] shrink-0 w-[413px]">
      <div aria-hidden="true" className="absolute border border-[#bfbed0] border-solid inset-0 pointer-events-none rounded-[12px]" />
      <Frame34 />
      <p className="font-['Lato:Regular',sans-serif] leading-[1.5] not-italic relative shrink-0 text-[#6b6b7b] text-[18px] text-center w-full">Conoce cómo funcionan nuestras garantías y devoluciones para proteger tu compra en todo momento.</p>
      <div className="bg-white h-[54px] relative rounded-[12px] shrink-0 w-full" data-name="Botones">
        <div className="flex flex-row items-center justify-center size-full">
          <div className="content-stretch flex gap-[4px] items-center justify-center p-[16px] relative size-full">
            <p className="font-['Lato:Regular',sans-serif] leading-[1.5] not-italic relative shrink-0 text-[#302c67] text-[16px] whitespace-nowrap">Leer más</p>
            <div className="relative shrink-0 size-[24px]" data-name="arrow_forward">
              <div className="absolute inset-[18.75%] mask-alpha mask-intersect mask-no-clip mask-no-repeat mask-position-[-4.5px_-4.5px] mask-size-[24px_24px]" data-name="arrow_forward" style={{ maskImage: `url('${imgVerified}')` }}>
                <svg className="absolute block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 15 15">
                  <path d={svgPaths.p5078080} fill="var(--fill-0, #302C67)" id="arrow_forward" />
                </svg>
              </div>
            </div>
          </div>
        </div>
      </div>
      <div className="h-[54px] relative rounded-[12px] shrink-0 w-full" data-name="Botones">
        <div aria-hidden="true" className="absolute border border-[#302c67] border-solid inset-0 pointer-events-none rounded-[12px]" />
        <div className="flex flex-row items-center justify-center size-full">
          <div className="content-stretch flex gap-[4px] items-center justify-center p-[16px] relative size-full">
            <p className="font-['Lato:Regular',sans-serif] leading-[1.5] not-italic relative shrink-0 text-[#302c67] text-[16px] whitespace-nowrap">Descargar</p>
            <div className="relative shrink-0 size-[24px]" data-name="arrow_forward">
              <div className="absolute inset-[18.75%] mask-alpha mask-intersect mask-no-clip mask-no-repeat mask-position-[-4.5px_-4.5px] mask-size-[24px_24px]" data-name="arrow_forward" style={{ maskImage: `url('${imgVerified}')` }}>
                <svg className="absolute block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 15 15">
                  <path d={svgPaths.p5078080} fill="var(--fill-0, #302C67)" id="arrow_forward" />
                </svg>
              </div>
            </div>
          </div>
        </div>
      </div>
    </div>
  );
}

function Frame35() {
  return (
    <div className="content-stretch flex flex-col gap-[4px] items-center relative shrink-0 w-full">
      <div className="relative rounded-[4px] shrink-0 size-[64px]" data-name="shield">
        <div className="absolute inset-[10.58%_18.75%_10.5%_18.75%] mask-alpha mask-intersect mask-no-clip mask-no-repeat mask-position-[-4.5px_-2.539px] mask-size-[24px_24px]" data-name="shield" style={{ maskImage: `url('${imgLocalShipping}')` }}>
          <svg className="absolute block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 40 50.5127">
            <path d={svgPaths.pc43c400} fill="var(--fill-0, #302C67)" id="shield" />
          </svg>
        </div>
      </div>
      <p className="font-['Lato:Bold',sans-serif] leading-[1.3] min-w-full not-italic relative shrink-0 text-[#262352] text-[24px] text-center w-[min-content]">Garantía</p>
    </div>
  );
}

function Frame8() {
  return (
    <div className="bg-white content-stretch flex flex-col gap-[12px] items-center justify-center p-[24px] relative rounded-[12px] shrink-0 w-[412px]">
      <div aria-hidden="true" className="absolute border border-[#bfbed0] border-solid inset-0 pointer-events-none rounded-[12px]" />
      <Frame35 />
      <p className="font-['Lato:Regular',sans-serif] leading-[1.5] not-italic relative shrink-0 text-[#6b6b7b] text-[18px] text-center w-full">Descubre cómo realizamos nuestros envíos, tiempos de entrega y opciones de seguimiento.</p>
      <div className="bg-white h-[54px] relative rounded-[12px] shrink-0 w-full" data-name="Botones">
        <div className="flex flex-row items-center justify-center size-full">
          <div className="content-stretch flex gap-[4px] items-center justify-center p-[16px] relative size-full">
            <p className="font-['Lato:Regular',sans-serif] leading-[1.5] not-italic relative shrink-0 text-[#302c67] text-[16px] whitespace-nowrap">Leer más</p>
            <div className="relative shrink-0 size-[24px]" data-name="arrow_forward">
              <div className="absolute inset-[18.75%] mask-alpha mask-intersect mask-no-clip mask-no-repeat mask-position-[-4.5px_-4.5px] mask-size-[24px_24px]" data-name="arrow_forward" style={{ maskImage: `url('${imgVerified}')` }}>
                <svg className="absolute block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 15 15">
                  <path d={svgPaths.p5078080} fill="var(--fill-0, #302C67)" id="arrow_forward" />
                </svg>
              </div>
            </div>
          </div>
        </div>
      </div>
      <div className="h-[54px] relative rounded-[12px] shrink-0 w-full" data-name="Botones">
        <div aria-hidden="true" className="absolute border border-[#302c67] border-solid inset-0 pointer-events-none rounded-[12px]" />
        <div className="flex flex-row items-center justify-center size-full">
          <div className="content-stretch flex gap-[4px] items-center justify-center p-[16px] relative size-full">
            <p className="font-['Lato:Regular',sans-serif] leading-[1.5] not-italic relative shrink-0 text-[#302c67] text-[16px] whitespace-nowrap">Descargar</p>
            <div className="relative shrink-0 size-[24px]" data-name="arrow_forward">
              <div className="absolute inset-[18.75%] mask-alpha mask-intersect mask-no-clip mask-no-repeat mask-position-[-4.5px_-4.5px] mask-size-[24px_24px]" data-name="arrow_forward" style={{ maskImage: `url('${imgVerified}')` }}>
                <svg className="absolute block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 15 15">
                  <path d={svgPaths.p5078080} fill="var(--fill-0, #302C67)" id="arrow_forward" />
                </svg>
              </div>
            </div>
          </div>
        </div>
      </div>
    </div>
  );
}

function Frame36() {
  return (
    <div className="content-stretch flex flex-col gap-[4px] items-center relative shrink-0 w-full">
      <div className="relative rounded-[4px] shrink-0 size-[64px]" data-name="restart_alt">
        <div className="absolute inset-[12.9%_18.75%_14.79%_18.75%] mask-alpha mask-intersect mask-no-clip mask-no-repeat mask-position-[-4.5px_-3.096px] mask-size-[24px_24px]" data-name="restart_alt" style={{ maskImage: `url('${imgLocalShipping}')` }}>
          <svg className="absolute block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 40 46.2767">
            <path d={svgPaths.p11095c00} fill="var(--fill-0, #302C67)" id="restart_alt" />
          </svg>
        </div>
      </div>
      <p className="font-['Lato:Bold',sans-serif] leading-[1.3] min-w-full not-italic relative shrink-0 text-[#262352] text-[24px] text-center w-[min-content]">Devoluciones</p>
    </div>
  );
}

function Frame9() {
  return (
    <div className="bg-white content-stretch flex flex-col gap-[12px] items-center justify-center p-[24px] relative rounded-[12px] shrink-0 w-[413px]">
      <div aria-hidden="true" className="absolute border border-[#bfbed0] border-solid inset-0 pointer-events-none rounded-[12px]" />
      <Frame36 />
      <p className="font-['Lato:Regular',sans-serif] leading-[1.5] not-italic relative shrink-0 text-[#6b6b7b] text-[18px] text-center w-full">Conoce tus derechos, condiciones y pasos para realizar cambios o devoluciones de forma rápida y sencilla.</p>
      <div className="bg-white h-[54px] relative rounded-[12px] shrink-0 w-full" data-name="Botones">
        <div className="flex flex-row items-center justify-center size-full">
          <div className="content-stretch flex gap-[4px] items-center justify-center p-[16px] relative size-full">
            <p className="font-['Lato:Regular',sans-serif] leading-[1.5] not-italic relative shrink-0 text-[#302c67] text-[16px] whitespace-nowrap">Leer más</p>
            <div className="relative shrink-0 size-[24px]" data-name="arrow_forward">
              <div className="absolute inset-[18.75%] mask-alpha mask-intersect mask-no-clip mask-no-repeat mask-position-[-4.5px_-4.5px] mask-size-[24px_24px]" data-name="arrow_forward" style={{ maskImage: `url('${imgVerified}')` }}>
                <svg className="absolute block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 15 15">
                  <path d={svgPaths.p5078080} fill="var(--fill-0, #302C67)" id="arrow_forward" />
                </svg>
              </div>
            </div>
          </div>
        </div>
      </div>
      <div className="h-[54px] relative rounded-[12px] shrink-0 w-full" data-name="Botones">
        <div aria-hidden="true" className="absolute border border-[#302c67] border-solid inset-0 pointer-events-none rounded-[12px]" />
        <div className="flex flex-row items-center justify-center size-full">
          <div className="content-stretch flex gap-[4px] items-center justify-center p-[16px] relative size-full">
            <p className="font-['Lato:Regular',sans-serif] leading-[1.5] not-italic relative shrink-0 text-[#302c67] text-[16px] whitespace-nowrap">Descargar</p>
            <div className="relative shrink-0 size-[24px]" data-name="arrow_forward">
              <div className="absolute inset-[18.75%] mask-alpha mask-intersect mask-no-clip mask-no-repeat mask-position-[-4.5px_-4.5px] mask-size-[24px_24px]" data-name="arrow_forward" style={{ maskImage: `url('${imgVerified}')` }}>
                <svg className="absolute block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 15 15">
                  <path d={svgPaths.p5078080} fill="var(--fill-0, #302C67)" id="arrow_forward" />
                </svg>
              </div>
            </div>
          </div>
        </div>
      </div>
    </div>
  );
}

function Frame10() {
  return (
    <div className="content-stretch flex items-center justify-between relative shrink-0 w-full">
      <Frame7 />
      <Frame8 />
      <Frame9 />
    </div>
  );
}

function Group9() {
  return (
    <div className="grid-cols-[max-content] grid-rows-[max-content] inline-grid leading-[0] place-items-start relative shrink-0">
      <div className="bg-[#302c67] col-1 content-stretch flex gap-[4px] h-[54px] items-center justify-center ml-0 mt-0 p-[16px] relative rounded-[12px] row-1 w-[258px]" data-name="Botones">
        <p className="font-['Lato:Regular',sans-serif] leading-[1.5] not-italic relative shrink-0 text-[16px] text-white whitespace-nowrap">Leer blog</p>
        <div className="relative shrink-0 size-[24px]" data-name="arrow_forward">
          <div className="absolute inset-[18.75%] mask-alpha mask-intersect mask-no-clip mask-no-repeat mask-position-[-4.5px_-4.5px] mask-size-[24px_24px]" data-name="arrow_forward" style={{ maskImage: `url('${imgVerified}')` }}>
            <svg className="absolute block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 15 15">
              <path d={svgPaths.p5078080} fill="var(--fill-0, white)" id="arrow_forward" />
            </svg>
          </div>
        </div>
      </div>
    </div>
  );
}

function Frame6() {
  return (
    <div className="bg-[#eaeaf0] relative rounded-[24px] shrink-0 w-full">
      <div className="flex flex-col items-center justify-center size-full">
        <div className="content-stretch flex flex-col gap-[24px] items-center justify-center px-[80px] py-[48px] relative w-full">
          <p className="font-['DM_Sans:Bold',sans-serif] font-bold leading-[1.15] min-w-full relative shrink-0 text-[#302c67] text-[56px] text-center tracking-[-0.0084px] w-[min-content]" style={{ fontVariationSettings: "'opsz' 14" }}>
            Compra mejor, elige con confianza
          </p>
          <p className="font-['Lato:Regular',sans-serif] leading-[1.25] min-w-full not-italic relative shrink-0 text-[#302c67] text-[32px] text-center tracking-[-0.0016px] w-[min-content]">Explora consejos, tendencias y recomendaciones para aprovechar al máximo cada compra.</p>
          <Group9 />
        </div>
      </div>
    </div>
  );
}

function Frame23() {
  return (
    <div className="content-stretch flex flex-[1_0_0] flex-col items-start justify-center min-h-px min-w-px relative">
      <p className="font-['Lato:Regular',sans-serif] leading-[1.5] not-italic relative shrink-0 text-[#302c67] text-[18px] w-full">Atención al cliente</p>
    </div>
  );
}

function Frame29() {
  return (
    <div className="content-stretch flex gap-[4px] items-center relative shrink-0 w-full">
      <div className="relative shrink-0 size-[24px]" data-name="mobile">
        <div className="absolute inset-[6.25%_18.75%_6.25%_22.92%] mask-alpha mask-intersect mask-no-clip mask-no-repeat mask-position-[-5.5px_-1.5px] mask-size-[24px_24px]" data-name="mobile" style={{ maskImage: `url('${imgVerified}')` }}>
          <svg className="absolute block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 14 21">
            <path d={svgPaths.p3e9f6380} fill="var(--fill-0, #302C67)" id="mobile" />
          </svg>
        </div>
      </div>
      <Frame23 />
    </div>
  );
}

function Frame24() {
  return (
    <div className="content-stretch flex flex-col items-start justify-center relative shrink-0 w-full">
      <p className="font-['Lato:Regular',sans-serif] leading-[1.5] not-italic relative shrink-0 text-[#302c67] text-[18px] w-full">(+591) 721-22210</p>
    </div>
  );
}

function Frame71() {
  return (
    <div className="content-stretch flex flex-[1_0_0] flex-col gap-[4px] items-start min-h-px min-w-px relative">
      <Frame29 />
      <Frame24 />
    </div>
  );
}

function Frame25() {
  return (
    <div className="content-stretch flex flex-[1_0_0] flex-col items-start justify-center min-h-px min-w-px relative">
      <p className="font-['Lato:Regular',sans-serif] leading-[1.5] not-italic relative shrink-0 text-[#302c67] text-[18px] w-full">Correo electrónico</p>
    </div>
  );
}

function Frame30() {
  return (
    <div className="content-stretch flex gap-[4px] items-center relative shrink-0 w-full">
      <div className="relative shrink-0 size-[24px]" data-name="mail">
        <div className="absolute inset-[18.75%_10.42%] mask-alpha mask-intersect mask-no-clip mask-no-repeat mask-position-[-2.5px_-4.5px] mask-size-[24px_24px]" data-name="mail" style={{ maskImage: `url('${imgVerified}')` }}>
          <svg className="absolute block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 19 15">
            <path d={svgPaths.p1bbbecb2} fill="var(--fill-0, #302C67)" id="mail" />
          </svg>
        </div>
      </div>
      <Frame25 />
    </div>
  );
}

function Frame26() {
  return (
    <div className="content-stretch flex flex-col items-start justify-center relative shrink-0 w-full">
      <p className="font-['Lato:Regular',sans-serif] leading-[1.5] not-italic relative shrink-0 text-[#302c67] text-[18px] w-full">ventas@biddam.com</p>
    </div>
  );
}

function Frame72() {
  return (
    <div className="content-stretch flex flex-[1_0_0] flex-col gap-[4px] items-start min-h-px min-w-px relative">
      <Frame30 />
      <Frame26 />
    </div>
  );
}

function Frame27() {
  return (
    <div className="content-stretch flex flex-[1_0_0] flex-col items-start justify-center min-h-px min-w-px relative">
      <p className="font-['Lato:Regular',sans-serif] leading-[1.5] not-italic relative shrink-0 text-[#302c67] text-[18px] w-full">BigDam</p>
    </div>
  );
}

function Frame31() {
  return (
    <div className="content-stretch flex gap-[4px] items-center relative shrink-0 w-full">
      <div className="relative shrink-0 size-[24px]" data-name="location_on">
        <div className="absolute inset-[10.42%_18.55%_10.38%_18.55%] mask-alpha mask-intersect mask-no-clip mask-no-repeat mask-position-[-4.452px_-2.5px] mask-size-[24px_24px]" data-name="location_on" style={{ maskImage: `url('${imgVerified}')` }}>
          <svg className="absolute block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 15.096 19.0095">
            <path d={svgPaths.p1d149900} fill="var(--fill-0, #302C67)" id="location_on" />
          </svg>
        </div>
      </div>
      <Frame27 />
    </div>
  );
}

function Frame28() {
  return (
    <div className="content-stretch flex flex-col items-start justify-center relative shrink-0 w-full">
      <p className="font-['Lato:Regular',sans-serif] leading-[1.5] not-italic relative shrink-0 text-[#302c67] text-[18px] w-full">Santa Cruz, Bolivia</p>
    </div>
  );
}

function Frame73() {
  return (
    <div className="content-stretch flex flex-[1_0_0] flex-col gap-[4px] items-start min-h-px min-w-px relative">
      <Frame31 />
      <Frame28 />
    </div>
  );
}

function Frame70() {
  return (
    <div className="content-stretch flex items-center justify-between relative shrink-0 w-full">
      <Frame71 />
      <Frame72 />
      <Frame73 />
    </div>
  );
}

function Frame11() {
  return (
    <div className="bg-white relative rounded-[12px] shrink-0 w-full">
      <div aria-hidden="true" className="absolute border border-[#bfbed0] border-solid inset-0 pointer-events-none rounded-[12px]" />
      <div className="flex flex-col items-center justify-center size-full">
        <div className="content-stretch flex flex-col items-center justify-center p-[48px] relative w-full">
          <Frame70 />
        </div>
      </div>
    </div>
  );
}

function Frame54() {
  return (
    <div className="absolute content-stretch flex flex-col gap-[48px] items-start left-[64px] top-[285px] w-[1312px]">
      <p className="font-['Lato:Bold',sans-serif] leading-[1.2] not-italic relative shrink-0 text-[#6b6b7b] text-[36px] tracking-[-0.0054px] w-[722px]">Preguntas frecuentes</p>
      <Frame1 />
      <Frame47 />
      <Frame46 />
      <Frame53 />
      <Frame62 />
      <p className="font-['Lato:Bold',sans-serif] leading-[1.2] min-w-full not-italic relative shrink-0 text-[#6b6b7b] text-[36px] tracking-[-0.0054px] w-[min-content]">Guía y recomendaciones</p>
      <Frame10 />
      <Frame6 />
      <p className="font-['Lato:Bold',sans-serif] leading-[1.2] min-w-full not-italic relative shrink-0 text-[#6b6b7b] text-[36px] tracking-[-0.0054px] w-[min-content]">Contacto</p>
      <Frame11 />
    </div>
  );
}

function Frame15() {
  return (
    <div className="content-stretch flex flex-col gap-[20px] h-full items-start relative shrink-0">
      <div className="content-stretch flex h-[44px] items-center relative shrink-0 w-[197px]" data-name="BD">
        <div className="h-[44.116px] relative shrink-0 w-[197px]" data-name="BIGDAM AZUL 1">
          <div className="absolute inset-0 overflow-hidden pointer-events-none">
            <img alt="" className="absolute h-[146.54%] left-[-1.54%] max-w-none top-[-23.27%] w-[102.87%]" src={imgBigdamAzul1} />
          </div>
        </div>
      </div>
      <div className="flex flex-col font-['Lato:Regular',sans-serif] justify-center leading-[0] not-italic relative shrink-0 text-[#6b6b7b] text-[14px] tracking-[0.0014px] w-[200px]">
        <p className="leading-[1.5]">© 2025 BigDam Repuestos Automotrices. Todos los derechos reservados</p>
      </div>
    </div>
  );
}

function Frame12() {
  return (
    <div className="content-stretch flex flex-col gap-[24px] h-full items-start leading-[0] not-italic relative shrink-0 text-[#6b6b7b] w-[240px]">
      <div className="flex flex-col font-['Lato:Bold',sans-serif] h-[20px] justify-center relative shrink-0 text-[24px] w-full">
        <p className="leading-[1.3]">Categorías</p>
      </div>
      <div className="flex flex-col font-['Lato:Regular',sans-serif] h-[20px] justify-center relative shrink-0 text-[16px] w-full">
        <p className="leading-[1.5]">Autopartes</p>
      </div>
      <div className="flex flex-col font-['Lato:Regular',sans-serif] h-[20px] justify-center relative shrink-0 text-[16px] w-full">
        <p className="leading-[1.5]">Electrónica</p>
      </div>
      <div className="flex flex-col font-['Lato:Regular',sans-serif] h-[20px] justify-center relative shrink-0 text-[16px] w-full">
        <p className="leading-[1.5]">Ferretería</p>
      </div>
      <div className="flex flex-col font-['Lato:Regular',sans-serif] h-[20px] justify-center relative shrink-0 text-[16px] w-full">
        <p className="leading-[1.5]">Ver más</p>
      </div>
    </div>
  );
}

function Frame13() {
  return (
    <div className="content-stretch flex flex-col gap-[24px] items-start relative shrink-0 w-[192px]">
      <div className="flex flex-col font-['Lato:Bold',sans-serif] h-[20px] justify-center relative shrink-0 text-[24px] w-full">
        <p className="leading-[1.3]">Quienes somos</p>
      </div>
      <div className="flex flex-col font-['Lato:Regular',sans-serif] h-[20px] justify-center relative shrink-0 text-[16px] w-full">
        <p className="leading-[1.5]">Sobre nosotros</p>
      </div>
      <div className="flex flex-col font-['Lato:Regular',sans-serif] h-[20px] justify-center relative shrink-0 text-[16px] w-full">
        <p className="leading-[1.5]">Blog</p>
      </div>
    </div>
  );
}

function Frame14() {
  return (
    <div className="content-stretch flex flex-col gap-[24px] items-start relative shrink-0 w-[192px]">
      <div className="flex flex-col font-['Lato:Bold',sans-serif] h-[20px] justify-center relative shrink-0 text-[24px] w-full">
        <p className="leading-[1.3]">Ayuda</p>
      </div>
      <div className="flex flex-col font-['Lato:Regular',sans-serif] h-[20px] justify-center relative shrink-0 text-[16px] w-full">
        <p className="leading-[1.5]">Centro de ayuda</p>
      </div>
    </div>
  );
}

function Frame32() {
  return (
    <div className="content-stretch flex flex-col gap-[24px] items-start leading-[0] not-italic relative shrink-0 text-[#6b6b7b] w-[240px]">
      <Frame13 />
      <Frame14 />
    </div>
  );
}

function Group1() {
  return (
    <div className="absolute inset-[0_7.44%_0_8.33%]" data-name="Group">
      <svg className="absolute block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 26.954 32.0001">
        <g id="Group">
          <path d={svgPaths.p27c03900} fill="var(--fill-0, #6B6B7B)" id="Vector" />
        </g>
      </svg>
    </div>
  );
}

function Group() {
  return (
    <div className="absolute contents inset-[0_7.44%_0_8.33%]" data-name="Group">
      <Group1 />
    </div>
  );
}

function AppleNegative() {
  return (
    <div className="overflow-clip relative shrink-0 size-[32px]" data-name="Apple---Negative 1">
      <Group />
    </div>
  );
}

function Info() {
  return (
    <div className="content-stretch flex flex-col gap-[4px] items-start not-italic relative shrink-0 text-[#6b6b7b]" data-name="Info">
      <p className="font-['Inter:Regular',sans-serif] font-normal leading-[13px] relative shrink-0 text-[11px] w-[88px]">Disponible en</p>
      <p className="font-['Inter:Semi_Bold',sans-serif] font-semibold leading-[20px] relative shrink-0 text-[14px] w-[88px]">App Store</p>
    </div>
  );
}

function Appstore() {
  return (
    <div className="bg-[#f0f0f2] content-stretch flex flex-col items-start relative rounded-[12px] shrink-0 w-full" data-name="Appstore">
      <div className="bg-[#f0f0f2] relative rounded-[8px] shrink-0 w-full" data-name="Mobile App">
        <div className="flex flex-row items-center justify-center size-full">
          <div className="content-stretch flex gap-[16px] items-center justify-center px-[20px] py-[16px] relative w-full">
            <AppleNegative />
            <Info />
          </div>
        </div>
      </div>
    </div>
  );
}

function Frame16() {
  return (
    <div className="content-stretch flex flex-col gap-[20px] items-start relative shrink-0 w-full">
      <div className="flex flex-col font-['Lato:Bold',sans-serif] h-[20px] justify-center leading-[0] not-italic relative shrink-0 text-[#6b6b7b] text-[24px] w-full">
        <p className="leading-[1.3]">Descarga nuestra app</p>
      </div>
      <Appstore />
    </div>
  );
}

function Group2() {
  return (
    <div className="absolute inset-[2.08%_5.68%_-2.08%_4.17%]" data-name="Group">
      <svg className="absolute block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 28.8472 32">
        <g id="Group">
          <path d={svgPaths.p2d1ad380} fill="var(--fill-0, #6B6B7B)" id="Vector" />
        </g>
      </svg>
    </div>
  );
}

function IconGooglePlay() {
  return (
    <div className="overflow-clip relative shrink-0 size-[32px]" data-name="Icon=google-play 1">
      <Group2 />
    </div>
  );
}

function Info1() {
  return (
    <div className="content-stretch flex flex-col gap-[4px] items-start not-italic relative shrink-0 text-[#6b6b7b]" data-name="Info">
      <p className="font-['Inter:Regular',sans-serif] font-normal leading-[13px] relative shrink-0 text-[11px] w-[88px]">Disponible en</p>
      <p className="font-['Inter:Semi_Bold',sans-serif] font-semibold leading-[20px] relative shrink-0 text-[14px] w-[88px]">Google Play</p>
    </div>
  );
}

function GooglePlay() {
  return (
    <div className="bg-[#f0f0f2] relative rounded-[12px] shrink-0 w-full" data-name="Google play">
      <div className="flex flex-row items-center justify-center size-full">
        <div className="content-stretch flex gap-[16px] items-center justify-center px-[20px] py-[16px] relative w-full">
          <IconGooglePlay />
          <Info1 />
        </div>
      </div>
    </div>
  );
}

function Frame33() {
  return (
    <div className="content-stretch flex flex-col gap-[24px] items-start relative shrink-0 w-[240px]">
      <Frame16 />
      <GooglePlay />
    </div>
  );
}

function Frame20() {
  return (
    <div className="content-stretch flex items-center justify-between relative shrink-0 w-full">
      <div className="flex flex-row items-center self-stretch">
        <Frame15 />
      </div>
      <div className="flex flex-row items-center self-stretch">
        <Frame12 />
      </div>
      <Frame32 />
      <Frame33 />
    </div>
  );
}

function Group3() {
  return (
    <div className="absolute contents inset-[20.35%_19.02%_20.34%_19.03%]" data-name="Group">
      <div className="absolute inset-[20.34%_19.02%_20.34%_19.03%]" data-name="Vector">
        <svg className="absolute block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 14.8677 14.2396">
          <path d={svgPaths.p3168cff0} fill="var(--fill-0, white)" id="Vector" />
        </svg>
      </div>
      <div className="absolute inset-[35.58%]" data-name="Vector">
        <svg className="absolute block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 6.92177 6.92381">
          <path d={svgPaths.p20fd7a40} fill="var(--fill-0, white)" id="Vector" />
        </svg>
      </div>
      <div className="absolute inset-[30.14%_30.36%_62.09%_61.87%]" data-name="Vector">
        <svg className="absolute block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 1.86443 1.86498">
          <path d={svgPaths.pea439c0} fill="var(--fill-0, white)" id="Vector" />
        </svg>
      </div>
    </div>
  );
}

function SocialMedia1() {
  return (
    <div className="content-stretch flex gap-[36.5px] h-[24.333px] items-center justify-center relative shrink-0" data-name="Social media">
      <div className="h-[24.006px] relative shrink-0 w-[24px]" data-name="Facebook">
        <svg className="absolute block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 24 24.0062">
          <path d={svgPaths.p8bcac00} fill="var(--fill-0, #F0F0F2)" id="Vector" />
        </svg>
        <div className="absolute inset-[22.74%_29.57%_0_29.56%]" data-name="Vector">
          <svg className="absolute block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 9.8097 18.5469">
            <path d={svgPaths.p236f0980} fill="var(--fill-0, #6B6B7B)" id="Vector" />
          </svg>
        </div>
      </div>
      <div className="h-[24.007px] relative shrink-0 w-[24px]" data-name="TikTok">
        <svg className="absolute block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 24 24.0071">
          <path d={svgPaths.p2cf92380} fill="var(--fill-0, #F0F0F2)" id="Vector" />
        </svg>
        <div className="absolute inset-[22.91%_25.83%_22.31%_26.71%]" data-name="Vector">
          <svg className="absolute block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 11.3908 13.1512">
            <path d={svgPaths.p2f8c3af2} fill="var(--fill-0, #6B6B7B)" id="Vector" />
          </svg>
        </div>
      </div>
      <div className="h-[24.007px] relative shrink-0 w-[24px]" data-name="Instagram">
        <svg className="absolute block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 24 24.0071">
          <path d={svgPaths.p2cf92380} fill="var(--fill-0, #F0F0F2)" id="Vector" />
        </svg>
        <Group3 />
      </div>
    </div>
  );
}

function Frame17() {
  return (
    <div className="content-stretch flex gap-[4px] items-start relative shrink-0 w-[340.67px]">
      <div className="relative shrink-0 size-[24px]" data-name="mobile">
        <div className="absolute inset-[6.25%_18.75%_6.25%_22.92%] mask-alpha mask-intersect mask-no-clip mask-no-repeat mask-position-[-5.5px_-1.5px] mask-size-[24px_24px]" data-name="mobile" style={{ maskImage: `url('${imgVerified}')` }}>
          <svg className="absolute block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 14 21">
            <path d={svgPaths.p3e9f6380} fill="var(--fill-0, #6B6B7B)" id="mobile" />
          </svg>
        </div>
      </div>
      <div className="flex flex-col font-['Lato:Regular',sans-serif] justify-center leading-[0] not-italic relative shrink-0 text-[#6b6b7b] text-[16px] whitespace-nowrap">
        <p className="leading-[1.5]">(+591) 721-22210</p>
      </div>
    </div>
  );
}

function Frame18() {
  return (
    <div className="content-stretch flex gap-[4px] items-start relative shrink-0 w-[340.67px]">
      <div className="relative shrink-0 size-[24px]" data-name="mail">
        <div className="absolute inset-[18.75%_10.42%] mask-alpha mask-intersect mask-no-clip mask-no-repeat mask-position-[-2.5px_-4.5px] mask-size-[24px_24px]" data-name="mail" style={{ maskImage: `url('${imgVerified}')` }}>
          <svg className="absolute block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 19 15">
            <path d={svgPaths.p1bbbecb2} fill="var(--fill-0, #6B6B7B)" id="mail" />
          </svg>
        </div>
      </div>
      <div className="flex flex-col font-['Lato:Regular',sans-serif] justify-center leading-[0] not-italic relative shrink-0 text-[#6b6b7b] text-[16px] whitespace-nowrap">
        <p className="leading-[1.5]">ventas@biddam.com</p>
      </div>
    </div>
  );
}

function Frame19() {
  return (
    <div className="content-stretch flex gap-[4px] items-start relative shrink-0">
      <div className="relative shrink-0 size-[24px]" data-name="location_on">
        <div className="absolute inset-[10.42%_18.55%_10.38%_18.55%] mask-alpha mask-intersect mask-no-clip mask-no-repeat mask-position-[-4.452px_-2.5px] mask-size-[24px_24px]" data-name="location_on" style={{ maskImage: `url('${imgVerified}')` }}>
          <svg className="absolute block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 15.096 19.0095">
            <path d={svgPaths.p1d149900} fill="var(--fill-0, #6B6B7B)" id="location_on" />
          </svg>
        </div>
      </div>
      <div className="flex flex-col font-['Lato:Regular',sans-serif] justify-center leading-[0] not-italic relative shrink-0 text-[#6b6b7b] text-[16px] whitespace-nowrap">
        <p className="leading-[1.5]">Santa Cruz, Bolivia</p>
      </div>
    </div>
  );
}

function SocialMedia() {
  return (
    <div className="content-stretch flex gap-[48px] h-[32px] items-center justify-center relative shrink-0" data-name="Social media">
      <SocialMedia1 />
      <Frame17 />
      <Frame18 />
      <Frame19 />
    </div>
  );
}

function Frame21() {
  return (
    <div className="content-stretch flex flex-col gap-[48px] items-center justify-center relative shrink-0 w-full">
      <div className="h-0 relative shrink-0 w-full">
        <div className="absolute inset-[-1px_0_0_0]">
          <svg className="block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 1240 1">
            <line id="Line 34" opacity="0.5" stroke="var(--stroke-0, #BFBED0)" x2="1240" y1="0.5" y2="0.5" />
          </svg>
        </div>
      </div>
      <SocialMedia />
    </div>
  );
}

function Frame22() {
  return (
    <div className="content-stretch flex flex-col h-[330px] items-center justify-between relative shrink-0 w-[1240px]">
      <Frame20 />
      <Frame21 />
    </div>
  );
}

function Group4() {
  return (
    <div className="absolute contents inset-[0_0_16.67%_0]">
      <div className="absolute bg-[#7ad26d] inset-[0_0_16.67%_0] rounded-bl-[5px] rounded-tl-[5px] rounded-tr-[5px]" />
    </div>
  );
}

function Group5() {
  return (
    <div className="absolute contents inset-[21.67%_30.77%_38.33%_32.31%]">
      <div className="absolute inset-[21.67%_30.77%_38.33%_32.31%]" data-name="Whatsapp">
        <div className="absolute inset-0 overflow-clip" data-name="Logos / whatsapp-line">
          <svg className="absolute block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 24 24">
            <g id="Group">
              <g id="Vector" />
              <path d={svgPaths.p1b9a6f80} fill="var(--fill-0, white)" id="Vector_2" />
            </g>
          </svg>
        </div>
      </div>
    </div>
  );
}

function Group6() {
  return (
    <div className="absolute contents inset-[0_0_16.67%_0]">
      <Group4 />
      <Group5 />
    </div>
  );
}

export default function CentroDeAyuda() {
  return (
    <div className="bg-white relative size-full" data-name="Centro de ayuda">
      <Group7 />
      <div className="absolute bottom-0 h-[4435px] left-0 pointer-events-none top-[48px]">
        <div className="bg-white content-stretch flex h-[80px] items-center justify-between pointer-events-auto px-[64px] py-[18px] sticky top-0 w-[1440px]" data-name="Header-web">
          <div className="content-stretch flex h-[44px] items-center relative shrink-0 w-[197px]" data-name="BD">
            <div className="h-[44.116px] relative shrink-0 w-[197px]" data-name="BIGDAM AZUL 1">
              <div className="absolute inset-0 overflow-hidden pointer-events-none">
                <img alt="" className="absolute h-[146.54%] left-[-1.54%] max-w-none top-[-23.27%] w-[102.87%]" src={imgBigdamAzul1} />
              </div>
            </div>
          </div>
          <Frame />
          <Group8 />
        </div>
      </div>
      <p className="absolute font-['DM_Sans:Bold',sans-serif] font-bold leading-[1.15] left-[64px] text-[#6b6b7b] text-[56px] top-[185px] tracking-[-0.0084px] w-[722px]" style={{ fontVariationSettings: "'opsz' 14" }}>
        ¿Cómo podemos ayudarte?
      </p>
      <div className="-translate-x-1/2 absolute h-0 left-1/2 top-[267px] w-[1312px]">
        <div className="absolute inset-[-1px_0_0_0]">
          <svg className="block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 1312 1">
            <line id="Line 35" stroke="var(--stroke-0, #BFBED0)" x2="1312" y1="0.5" y2="0.5" />
          </svg>
        </div>
      </div>
      <Frame54 />
      <div className="-translate-x-1/2 absolute bg-white bottom-0 content-stretch flex flex-col items-center left-1/2 px-[64px] py-[48px] w-[1440px]" data-name="Footer-web">
        <div aria-hidden="true" className="absolute border-[#bfbed0] border-solid border-t inset-0 pointer-events-none" />
        <Frame22 />
      </div>
      <div className="absolute h-[60px] left-[calc(91.67%+1px)] top-[2627px] w-[65px]">
        <Group6 />
        <div className="absolute flex inset-[83.33%_0_0_84.62%] items-center justify-center">
          <div className="-rotate-90 flex-none size-[10px]">
            <div className="relative size-full">
              <div className="absolute inset-[0_0_0_14.12%]">
                <svg className="block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 8.58779 10">
                  <path d={svgPaths.p21569a80} fill="var(--fill-0, #7AD26D)" id="Polygon 8" />
                </svg>
              </div>
            </div>
          </div>
        </div>
      </div>
    </div>
  );
}